...
chrome            — kasmweb/chrome (VNC :6901 + CDP, worker のサイドカー)
preprocessor      — inbox → task 変換
worker            — タスク処理（Chrome と localhost 共有、VNC ポート公開）
...
claude-inbox-setup nlm-login   # NotebookLM 認証（Chrome サイドカー + CDP、VNC でログイン）
...
| `chrome` | kasmweb/chrome:1.15.0 (VNC :6901 + CDP, worker のサイドカー) | `chrome_profile` |
| `preprocessor` | inbox → task 変換 | inbox |
| `worker` | タスク処理（Chrome と localhost 共有、VNC ポート公開） | `claude_dot`, inbox, workdir, `nlm_auth`, `gws_config` |
...
- Chrome は worker のサイドカー (`network_mode: "service:worker"`) で localhost 共有
- CDP は `http://localhost:9222` で直接接続（socat relay 不要）
...
認証: `bin/claude-inbox-setup nlm-login`（Chrome サイドカー + CDP 経由、VNC でログイン操作）。Cookie ベース、2-4週間持続。credentials は `nlm_auth` volume に永続化。
...
| ブラウザ操作 | kasmweb/chrome サイドカー (localhost 共有) | worker 内蔵 Chrome / CDP relay | Chrome 分離でイメージ軽量化 + localhost 共有で OAuth コールバック直通 |
