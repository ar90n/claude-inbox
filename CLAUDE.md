...
├── skills/
│   ├── browser-use/SKILL.md      # AI ブラウザ自動操作
...
claude-inbox-setup nlm-login   # NotebookLM 認証（Chrome + CDP 経由）
...
| `worker` | タスク処理 × N | `claude_dot`, inbox, workdir |
...
`browser-use` + Playwright + Claude でブラウザを AI 操作。

- Google Chrome をコンテナ内に同梱（`/usr/bin/google-chrome-stable`）
- `ANTHROPIC_API_KEY` が必要（Claude Code の認証とは別）
- `CHROME_PROFILE_DIR` をマウントすれば既存の Google ログイン状態を使用可能
- X11 転送 (`/tmp/.X11-unix` マウント + `DISPLAY`) で可視ブラウザも可
...
認証: `bin/claude-inbox-setup nlm-login`（Chrome + CDP 経由）。Cookie ベース、2-4週間持続。
...
| ブラウザ操作 | browser-use + system Chrome | Playwright のみ | AI 駆動の方が汎用的 |
...
**オプション機能（.env + docker-compose.yml のコメントアウト解除）:**

| 機能 | 設定 |
|---|---|
| Chrome プロファイル（browser-use） | `CHROME_PROFILE_DIR=~/.config/google-chrome` + volume |
| X11 転送（可視ブラウザ / nlm-login） | `DISPLAY` env + `/tmp/.X11-unix` volume |
